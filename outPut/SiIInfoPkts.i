...
#line 1 "sii9135\SiIInfoPkts.c" /0
...
#line 14 "sii9135\SiIInfoPkts.c" /0
...
#line 16 "sii9135\SiIInfoPkts.c" /0
...
#line 17 "sii9135\SiIInfoPkts.c" /0
...
#line 18 "sii9135\SiIInfoPkts.c" /0
...
#line 19 "sii9135\SiIInfoPkts.c" /0
...
#line 20 "sii9135\SiIInfoPkts.c" /0
...
#line 21 "sii9135\SiIInfoPkts.c" /0
 ...
 static bit Check_CheckSum( BYTE bTotInfoFrameLength , BYTE * pbInfoData ){
  bit qResult = 0;
 BYTE i, bCheckSum = 0;
 ...
 static InfoFrPktType GetPacketTypeAndAddr( BYTE bMaskedIntrrupt ) {
 InfoFrPktType InfoFrmPkt;
 ...
 case AVI_Mask:      InfoFrmPkt.bAddr = 0x40;
 break;
 case SPD_Mask:      InfoFrmPkt.bAddr = 0x60;
 break;
 case Audio_Mask:    InfoFrmPkt.bAddr = 0x80;
 break;
 case MPEG_Mask:     InfoFrmPkt.bAddr = 0xA0;
 break;
 case Unknown_Mask:  InfoFrmPkt.bAddr = 0xC0;
 break;
 case ACP_Mask:      InfoFrmPkt.bAddr = 0xE0;
 break;
 
 }
 if ( InfoFrmPkt.bAddr )
 InfoFrmPkt.bType = hlReadByte_8BA( 0x68, InfoFrmPkt.bAddr );
 return InfoFrmPkt;
 ...
 static bit CheckForInfoFrameLength( BYTE bInfoFrameLenth ){
  bit qResult = 0;
 
 if ( (bInfoFrameLenth >= 10) && (bInfoFrameLenth <= 27))
 qResult = 1;
 return qResult;
 ...
 case AVI_Type:      SiI_Inf.bNewInfoPkts &= (~SiI_RX_NewInfo_AVI);      break;
 case SPD_Type:      SiI_Inf.bNewInfoPkts &= (~SiI_RX_NewInfo_SPD);      break;
 case Audio_Type:    SiI_Inf.bNewInfoPkts &= (~SiI_RX_NewInfo_AUD);      break;
 case ISRC1_Type:    SiI_Inf.bNewInfoPkts &= (~SiI_RX_NewInfo_ISRC1);    break;
 case ISRC2_Type:    SiI_Inf.bNewInfoPkts &= (~SiI_RX_NewInfo_ISRC2);    break;
 case ACP_Type:      SiI_Inf.bNewInfoPkts &= (~SiI_RX_NewInfo_ACP);      break;
 ...
 static BYTE GetInfoFramesOrPakets( InfoFrPktType InfoFrPkt, BYTE * pbInfoData ){
 BYTE bECode = 0, bMaxNBytes = 0;
  bit qInfoFrame = 1;
 
 
 switch ( InfoFrPkt.bType ){
 case AVI_Type:    bMaxNBytes = 19;
 break;
 case MPEG_Type:
 case SPD_Type:    bMaxNBytes = 31;
 break;
 case Audio_Type:  bMaxNBytes = 14;
 break;
 case ISRC1_Type:
 case ISRC2_Type:
 case ACP_Type:
 qInfoFrame = 0; bMaxNBytes = 31;
 break;
 ...
 if ( qInfoFrame ) {
 if ( CheckForInfoFrameLength( pbInfoData[ 2 ] ) ){
 if ( !Check_CheckSum( pbInfoData[ 2 ] + 4, pbInfoData ) )
 bECode = SiI_EC_InfoFrameWrongCheckSum;
 }
 else bECode = SiI_EC_InfoFrameWrongLength;
 }
 ...
 case AVI_Type:      bAddr = 0x40;
 break;
 case SPD_Type:      bAddr = 0x60;
 break;
 case Audio_Type:    bAddr = 0x80;
 break;
 case ISRC1_Type:    bAddr = 0xA0;
 break;
 case ISRC2_Type:    bAddr = 0xC0;
 break;
 case ACP_Type:      bAddr = 0xE0;
 break;
 default:   bAddr = 0;
 }
 return bAddr;
 ...
 BYTE siiGetVIC ( void ) {
 return hlReadByte_8BA( 0x68, 0x40 + 7 );
 }
 
 
 
 
 static void SetAVI_Info( AVIType * AVI, BYTE * Data ){
 BYTE bAVI;
 ...
 AVI->bPixRepl = Data[4 + 4] & PixReplicationMask;
 ...
 void siiClearAVI_Info( AVIType * AVI ){
 ...
 InfoFrmPkt = GetPacketTypeAndAddr( bInfoFramesIntrrupts & AVI_Mask);
 if( InfoFrmPkt.bType ){
 ...
 InfoFrmPkt = GetPacketTypeAndAddr( bInfoFramesIntrrupts & 0x04);
 if ( InfoFrmPkt.bType ){
 bECode = GetInfoFramesOrPakets( InfoFrmPkt, abInfoData );
 if ( !bECode ){
 ...
 if ( ( bInfoFramesIntrrupts &  NoAVI_Mask )||( bInfoFramesIntrrupts &  AVI_Mask) ) {  
