...
#line 21 "sii9135\SiIISR.c" /0
 
  
#line 1 "sii9135\SiITTInfoPkts.h" /0
...
#line 23 "sii9135\SiIISR.c" /0
 
  
#line 1 "sii9135\SiITTasks.h" /0
...
#line 29 "sii9135\SiIISR.c" /0
 ...
 siiReadBlockHDMIRXP0( 0x71 - 1, 5, abIntrData );   
 siiReadBlockHDMIRXP0( 0x7B, 2, &abIntrData[5] );
 printf ("\n RX ints:");
 for ( i = 0; i < 7; i++ )
 ...
 static void SetNoAVIIntrrupts( bit qOn ){
 if ( qOn ) {
  siiModifyBits( 0x60, 0x78, 0x10, 1);
 SiI_Ctrl.bIgnoreIntr |= qcIgnoreNoAVI;
 }
 else {
  siiModifyBits( 0x60, 0x78, 0x10, 0);
 SiI_Ctrl.bIgnoreIntr &= (~qcIgnoreNoAVI);
 }
 }
 ...
 if (( *bpInfoFramesIntrrupts & NoAVI_Mask ) && ( *bpInfoFramesIntrrupts & AVI_Mask ) ){
 if ( CheckAVIHader() )
...
#line 141 "sii9135\SiIISR.c" /0
 ...
 static void EnableCheckHDCPFailureWithVSyncRate ( bit qOn ){
 
 if ( qOn ){
 SiI_Ctrl.bHDCPFailFrmCnt = 1;
 SiI_Ctrl.bIgnoreIntr |= qcIgnoreHDCPFail;  
 
  siiModifyBits( 0x60, 0x76, 0x40, 1);
  siiModifyBits( 0x60, 0x78, 0x40, 0);
 }
 else {
 SiI_Ctrl.bHDCPFailFrmCnt = 0;
 SiI_Ctrl.bIgnoreIntr &= (~qcIgnoreHDCPFail);
  siiModifyBits( 0x60, 0x76, 0x40, 0);
  siiModifyBits( 0x60, 0x78, 0x40, 1);
 }
 ...
 wHDCPErrors =   hlReadWord_8BA( 0x60, 0xCD );
 if ( !wHDCPErrors ){
 qError = 1;
 }
 return qError;
 ...
 EnableCheckHDCPFailureWithVSyncRate( 0 );
 }
 }
 else {
 EnableCheckHDCPFailureWithVSyncRate( 0 );
 ...
 siiClearBCHCounter();
 bError = GotNConsecutiveHDCPStuckInterrupt();
 EnableCheckHDCPFailureWithVSyncRate( 1 );
 }
 }
 return bError;
 }
 ...
 static void HandleDeepColorError ( void ) {
  hlWriteByte_8BA( 0x60, 0x7C, 0x80 );
 }
 ...
 siiReadBlockHDMIRXP0( 0x7B, 2, abIntrData );  
 
 
 
 
 abIntrData[1] &= ( ~0x04 );   
 siiWriteBlockHDMIRXP0( 0x7B, 2, abIntrData);
 
 if (( abIntrData[0] & 0x40) && (!(SiI_Ctrl.bIgnoreIntr & qcIgnoreAAC)) ) {
 siiTurnAudioOffAndSetSM_AudioOff();
  hlWriteByte_8BA( 0x60, 0x7B, 0x40 );
 
 printf ( "\nAAC");
 
 }
 
 if ( ( abIntrData[0] & 0x08) || ( abIntrData[0] & 0x10 ) ) {
 ...
 if ( abIntrData[1] & 0x02 ) {  
 SiI_Ctrl.sm_bAudio = SiI_RX_AS_ReqAudio;
 siiChangeDSDAudioStreamHandler ();
 }
 
 
 if (( SiI_Ctrl.bDevId == RX_SiI9135|| SiI_Ctrl.bDevId ==  RX_SiI9125 )&&( abIntrData[1] & 0x10 )){
 SiI_Ctrl.sm_bAudio = SiI_RX_AS_ReqAudio; 
 siiChangeHBRAudioStreamHandler ();		
 }
 
 if ( abIntrData[1] & 0x80 ) {
 HandleDeepColorError ();
 }
 
 
 
 siiReadBlockHDMIRXP0( 0x71, 4, abIntrData);  
 abIntrData[0] &= ( ~0x80 );   
 abIntrData[1] &= ( ~( 0x02 | 0x04) );   
 
 if ( SiI_Ctrl.bIgnoreIntr & qcIgnoreHDCPFail ) {
 if ( !(abIntrData[1] & 0x40) ) {
 abIntrData[3] &= ( ~0x40 );  
 }
 }
 
 siiWriteBlockHDMIRXP0( 0x71, 4, abIntrData);  
 
 if ( abIntrData[1] & 0x08 ) {
 EnableCheckHDCPFailureWithVSyncRate( 0 );
 siiMuteVideoAndSetSM_SyncInChange();
...
#line 326 "sii9135\SiIISR.c" /1
 
 
 
 
 
#line 331 "sii9135\SiIISR.c" /0
 }
 ...
 if ( abIntrData[0] & 0x02) {
 EnableCheckHDCPFailureWithVSyncRate( 0 );
 }
 
 if ( ( abIntrData[3] & 0x40 ) || ( SiI_Ctrl.bIgnoreIntr & qcIgnoreHDCPFail ) ) {
 bError = HDCPErrorHandler ( abIntrData[3] & 0x40, abIntrData[1] & 0x40 );
 }
 
 if ( ( abIntrData[2] & 0x1F) || ( abIntrData[3] & 0x10) ) {
 if ( ( abIntrData[3] & 0x10 ) && ( SiI_Ctrl.bIgnoreIntr & qcIgnoreNoAVI ) ) {
...
#line 386 "sii9135\SiIISR.c" /0
 
 
 
 
 static bit CheckRXInt_Bit( void ){
  bit qResult = 0;
 
 if ( hlReadByte_8BA( 0x60, 0x70 ) & 0x01 )
 qResult = 1;
 return qResult;
 }
