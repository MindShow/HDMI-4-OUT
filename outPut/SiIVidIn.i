...
#line 65 "sii9135\SiIVidIn.c" /0
 siiModifyBits( 0x60, 0x09, ( 0x01 | 0x10 | 0x02 | 0x20 ), 0);
 
 if ( bVideoInputChannels == SiI_RX_VInCh1 )
  siiModifyBits( 0x60, 0x09, 0x01 | 0x10, 1);
 else
  siiModifyBits( 0x60, 0x09, 0x02 | 0x20, 1);
 ...
 ChangeTMDSCoreAndDDC( bVideoInputChannel );     
 ...
 if ( ( SiI_Ctrl.bDevId ==  RX_SiI9011 ) && ( bVideoInputChannel != SiI_RX_VInCh1) ) {
 siiWarningHandler( SiI_WC_ChipNoCap );
 }
 else {
 
 
 if ( siiIsVideoOn() && ( SiI_Inf.bGlobStatus & SiI_RX_GlobalHDMI_Detected ) ) {
